...
# fill list with super-object IDS from features (check for NULLs). If in backup mode, set all super-object IDs to 0
if backup == True:
...
#----------define interval covering largest area and compute mean of attributes of all features in that interval as reference value (for each super-object separately) then compute difference to the reference for each feature -----------------
